...
export default async function PlatformPathPage() {
  ...
  const { data: settings } = await supabase.from('newsletter_settings').select('*').eq('slug', 'platform-path').single()
  const { data: articles } = await supabase
    .from('content').select('*').eq('published', true).eq('newsletter', 'platform-path')
    ...
    <div className="min-h-screen bg-bg text-cream">
      {settings?.banner_url && (
        <div className="w-full overflow-hidden">
          <img src={settings.banner_url} alt="The Platform Path" className="w-full object-cover" />
        </div>
      )}
      <div className="max-w-4xl px-6 md:px-16 py-16">
        <a href="https://www.linkedin.com/build-relation/newsletter-follow?entityUrn=7275180080509673473" target="_blank" rel="noopener noreferrer"
          ...
          <div key={year} id={`year-${year}`} className="mb-16 scroll-mt-8">
            ...
            {monthOrder.filter(m => months[m]).map((month) => (
              <div key={month} className="mb-10">
                <h3 className="text-lg font-serif text-cream/80 mb-5 uppercase tracking-wide text-sm">{month}</h3>
